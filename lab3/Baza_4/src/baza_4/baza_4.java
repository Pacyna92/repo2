...
public class baza_4 {
    ...
    public ArrayList<String> wyszukaj_zakupy_wg_miasta(String city) throws SQLException {
        polecenie = polaczenie.createStatement();
        sql = "SELECT * FROM CUSTOMER, PURCHASE_ORDER WHERE CUSTOMER.CITY = '" + city + "' AND "
                + "CUSTOMER.CUSTOMER_ID = PURCHASE_ORDER.CUSTOMER_ID";
        krotki = polecenie.executeQuery(sql);
        ArrayList<String> zakupy_wg_miasta = new ArrayList();
        zakupy_wg_miasta.add("\nLista wedlug miast\n");
        while(krotki.next()) {
            zakupy_wg_miasta.add(krotki.getString("NAME") + "\t"
                    + krotki.getString("CITY") + "\t"
                    + krotki.getString("CREDIT_LIMIT"));
            zakupy_wg_miasta.add("\n");
        }
        polecenie.close();
        return zakupy_wg_miasta;
    }
...
//            lista = baza.dane_zakupy_osob();
            lista = baza.wyszukaj_zakupy_wg_miasta(WeWy.weString("Podaj miasto: "));
            WeWy.wyString(lista);
        } catch (SQLException e) {
